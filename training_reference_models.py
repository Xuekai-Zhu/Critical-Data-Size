...
def main(args):
    ...
    # )
    dataset = load_dataset(
        args.dataset_name, cache_dir="/data1/xkzhu/datasets/cache/", num_proc=8
    )
    if args.sample_num != 0:
        dataset["train"] = dataset["train"].select(range(args.sample_num))
    ...
    def wikitext_process(examples):
        # examples["text"] = [" ".join([conv["value"] for conv in conv_list]) for conv_list in examples["conversations"]]
        batch = tokenizer(examples["text"], truncation=True, max_length=256, padding="max_length")
    ...
    config = transformers.AutoConfig.from_pretrained(args.base_model_name, local_files_only=True)
